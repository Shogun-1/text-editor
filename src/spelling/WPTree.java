...
public class WPTree implements WordPath {
	...
	public List<String> findPath(String word1, String word2) 
	{
        ...
        Queue<WPTreeNode> wordsToExplore = new LinkedList<>();
        List<String> visitedWords = new LinkedList<>();
        root = new WPTreeNode(word1, null);
        visitedWords.add(word1);
        wordsToExplore.add(root);
        ...
        while (!wordsToExplore.isEmpty()) {
            ...
            WPTreeNode curr = wordsToExplore.element();
            wordsToExplore.remove();
            List<String> neighbors = nw.distanceOne(curr.getWord(), true);
            for (String neighbor : neighbors) {
                if (!visitedWords.contains(neighbor)) {
                    visitedWords.add(neighbor);
                    wordsToExplore.add(curr.addChild(neighbor));
                    if (neighbor.equals(word2)) {
                        return curr.addChild(neighbor).buildPathToRoot();
                    }
                }
            }
            thresholdCounter++;
